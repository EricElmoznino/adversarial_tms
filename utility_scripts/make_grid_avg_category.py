...
save_dir = '/home/eric/Documents/experiments/adversarial_tms/gan_manipulations/grids/bold5000/scenecats/conv3'
n_samples = 10
...
categories = os.listdir(orig_dir)
...
for category_name in categories:
    ...
    orig_images = utils.listdir(os.path.join(orig_dir, category_name))
    for i in range(n_samples):
        orig_image = load_image(orig_images[i])
    ...
    image_title = make_image_title(category_name)
    grid = concat_v(image_title, grid, 0)
    grid.save(os.path.join(save_dir, category_name + '.jpg'))
